...
test('decode INT16 reads negative values at the correct offset', () => {
    ...
    const entry = { key: 'air_supply', addr: 8, len: 1, type: 'INT16', label: 'Supply air' };
...
test('decode UINT16 reads values above 32767', () => {
    ...
    const response = { body: { valuesAsBuffer: buffer } };
    const entry = { key: 'x', addr: 1, len: 1, type: 'UINT16', label: 'x' };
...
test('decode coil picks the right bit from a block', () => {
    ...
    const entry = { key: 'eco_mode', addr: 40, len: 1, type: 'UINT32', label: 'eco Mode' };
    assert.equal(decode(response, entry, 30, 'coil').value, '1');
    const off = { key: 'heating_coil', addr: 54, len: 1, type: 'UINT32', label: 'coil' };
...
test('decode returns xxx for unknown types', () => {
    ...
    const entry = { key: 'x', addr: 0, len: 1, type: 'BOGUS', label: 'x' };
